...
%             this.id, neighborsOri;
        ...
        function gattVel = calculate_gattVel(this, locations)
            goalAttVel = this.flock.goal - locations(1:2, this.id);
            gattVel = goalAttVel / norm(goalAttVel);
        end
...
%           Wall repulsion is given priority over attraction & orientation  
            if norm(wallRepVel) ~= 0
                ...
                % desiredVelocity = (attVelocity * W_att + oriVelocity * W_ori + this.isLeader * gattVelocity * W_gatt) / (W_gatt * this.isLeader + W_ori + W_att);
                desiredVelocity = (attVelocity * W_att + oriVelocity * W_ori) / (W_ori + W_att);
                if this.isLeader
                   desiredVelocity = W_gatt * gattVelocity + (1-W_gatt) * desiredVelocity;  
                end
            end
            ...
            if DEBUG
                ...
                if norm(oriVelocity) ~= 0
                    normd = oriVelocity;
                            W_att = 0.5;
            W_ori = 0.5;
            W_gatt = 0.6;    xpos = locations(1, this.id);
                    ypos = locations(2, this.id);
...
%                 end
        ...
        function result = sensesGoal(this, goal, senseGoalRadius)
            ...
            if this.knowsGoal
                result = true;
            elseif (norm(locations(1:2, this.id) - goal)) < senseGoalRadius && this.isLeader
                this.knowsGoal = true;
                result = true;
            else
                result = false;
