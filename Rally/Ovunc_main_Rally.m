...
informed_ar = zeros(1, experiments);
model_ar = cell(1, experiments);
nTop_ar = zeros(1, experiments);

% Parse Input File ----------------------------------------------------
for i=1:experiments
    fid=fopen(['../Input/Rally/', experimentSet, '/RallyInput', num2str(i), '.csv'],'rt');
    input = textscan(fid, '%f %f %f %f %f %f %f %f %s %f %f','HeaderLines',1,'Delimiter',',');
    initPos_ar(:,:,i) = cell2mat([input(:,1); input(:,2)]');
    c = cell2mat([input(1,3) input(1,4)]);
    goalPos_ar(:,:,i) = c(1,:);
    c = cell2mat(input(1,5));
    informed_ar(1,i) = c(1,1);
    c = cell2mat([input(1,6) input(1,7) input(1,8)]);
    radii_ar(:,:,i) = c(1,:);
    c = input(1,9);
    c = c{1}{1};
    model_ar{1,i} = c(1);
...
for i=1:experiments
    ...
    flock.assignLeaders(informed_ar(i));  % Percentage of leaders!
